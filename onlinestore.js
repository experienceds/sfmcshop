...
function sessionStart() {
    if (JSON.parse(sessionStorage.getItem("price") === null)) {
        ...
        if (deliveryLock[0] == 2 && total !== 0) {
            ...
            $("#deliveryOption").show();
            $("#delivShow").html("Delivery: 49.99")
            let num = (15 * (total + 49.99) / 100);
            $("#vat").html("VAT: " + num.toFixed(2));
        }
        $("#discShow").html("Discount: -" + discountLock);
...
$("#discount").click(function() {
    ...
    if ($("#code").val() == "vinyl" && $("#total").html() != "Total: 0") {
        discountCode.push("vinyl");
    } else if ($("#total").html() != "Total: 0") {
        alert("Code Not Valid!")
    }
    if (discountCode.length === 1 && $("#total").html() != "Total: 0") {
        discountLock = 20;
        alert("Discount Applied!");
        discountCode.push(1);
    } else if (discountCode.length > 1 && $("#code").val() == "vinyl" && $("#total").html() != "Total: 0") {
